...
function getPrompt(): string {
  const homeDir = currentDir.replace("~", "/home/vladimir");
  return `vladimir@kolchurin:${currentDir}$`;
}
  ...
  switch (cmd.toLowerCase()) {
    ...
    case "cat": {
      ...
      if (!filename) {
        ...
        lines.push({ type: "code", content: fileContents[currentDir + "/" + filename] });
      } else {
...
Try: cat skills.json, cat experience.json, cat about.txt`,
      });
      break;
...
<div class="terminal" onclick={focusInput} onkeydown={(e) => e.key === 'Enter' && focusInput()} role="button" tabindex="-1">
