...
const getUI = () => (window as unknown as { ui: (action: string, value?: string) => string }).ui
...
function updateBtnIcon() {
  if (themeBtn) {
    ...
    if (icon) icon.textContent = themeMode === "dark" ? "light_mode" : "dark_mode"
  }
}
</script>
...
<button
  bind:this={themeBtn}
  type="button"
  class="theme-toggle round padding"
  onclick={toggleTheme}
